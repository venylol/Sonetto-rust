// -----------------------------------------------------------------------------
// STUB MODULE (source checkout convenience)
// -----------------------------------------------------------------------------
//
// This repository expects `wasm-pack` build artifacts in `web/pkg/`:
//   - sonetto_wasm.js
...
//
// To build the real WASM engine, run:
...
function buildMissingWasmError(extra) {
  const msg = [
    'Sonetto WASM artifacts are missing.',
    'Build them with:',
    '  cd crates/sonetto_wasm',
    '  wasm-pack build --release --target web --out-dir ../../web/pkg',
