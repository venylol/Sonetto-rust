// -----------------------------------------------------------------------------
// STUB MODULE (WASM threads + SIMD build placeholder)
// -----------------------------------------------------------------------------
//
// This repository expects `wasm-pack` build artifacts in `web/pkg_threads_simd/`:
//   - sonetto_wasm.js
...
//   - 404 for ./pkg_threads_simd/sonetto_wasm.js
...
//
// To build the real WASM engine (threads + simd128), run:
//   cd crates/sonetto_wasm
//   RUSTFLAGS="-C target-feature=+simd128,+atomics,+bulk-memory,+mutable-globals" wasm-pack build --release --target web --out-dir ../../web/pkg_threads_simd --features "wasm_simd wasm-bindgen-rayon"
//
// When you build, wasm-pack will overwrite this file with the real loader.

function buildMissingWasmError(extra) {
  const msg = [
    'Sonetto WASM (threads + SIMD) artifacts are missing.',
    'Build them with:',
    '  cd crates/sonetto_wasm',
    '  RUSTFLAGS="-C target-feature=+simd128,+atomics,+bulk-memory,+mutable-globals" wasm-pack build --release --target web --out-dir ../../web/pkg_threads_simd --features "wasm_simd wasm-bindgen-rayon"',
